...
fn reasoning_tier_output_is_agnostic() {
    ...
    let out = render_skill(SCAFFOLDED_SKILL, Tier::Reasoning);
    assert!(
...
fn lean_tier_output_is_agnostic() {
    let out = render_skill(SCAFFOLDED_SKILL, Tier::Lean);
...
fn rendering_ignores_any_client_notion() {
    ...
    let a = render_skill(SCAFFOLDED_SKILL, Tier::Lean);
    let b = render_skill(SCAFFOLDED_SKILL, Tier::Lean);
    assert_eq!(a, b);
}
