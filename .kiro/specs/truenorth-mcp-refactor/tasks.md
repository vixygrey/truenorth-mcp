...
the destructive repository cleanup is sequenced last and gated on the crate + npm wrapper
...
- [ ] 1. Rust crate scaffold and stdio server boot
  ...
  - Ensure startup terminates with a non-zero exit status and an error indication if the stdio transport fails to initialize
  - _Requirements: 1.1, 1.2, 1.3_

- [ ] 2. Engine config — root resolution, denylist, git scope, sandbox config
  - [ ] 2.1 Implement `engine::config` (`config.rs`)
    - `get_repo_root()` evaluating candidates in order: `TRUENORTH_ROOT` env (when set and non-empty) → cwd → parent-of-package, selecting the first that directly contains both `skills/` and `specs/`
...
- [ ] 3. Engine spec models and backward-compat validation
  ...
  - [ ]* 3.3 Write property test for state preservation
    ...
    - Round-trip and mutate real `specs/*.yaml` fixtures; assert every unknown field (incl. `bigpowers_version`) is preserved with original key and value and the post-state still validates
...
- [ ] 4. Engine tier transforms
  ...
  - [ ]* 4.2 Write property/golden test for tier transforms
    ...
    - Golden fixtures asserting `full == md` identity; assert reasoning/lean retain invariant and acceptance-criterion statements and remove only meta/guardrail (and, for lean, rationale/background/verbose examples within budget)
...
- [ ] 5. Engine gate runner (sandbox)
  - [ ] 5.1 Implement `engine::gate_runner`
    ...
    - Wall-clock timeout (default 300s) with hard kill; return `{passed:true}` iff exit code 0 within timeout; on non-zero exit return an error with at most the final 2 KB of stderr plus remediation hints; on timeout return a timeout error with reduce-scope/raise-timeout hints; on allowlist miss return an error without executing
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6_

  - [ ]* 5.2 Write property/unit tests for the gate runner
    - **Property 2: verify_gate passes only on exit 0**
    - Using a fake command runner, cover pass (exit 0), fail (non-zero, stderr tail ≤ 2 KB), timeout (hard kill), allowlist-reject (no execution), and evidence-only paths
...
- [ ] 6. Engine ontology scan
  - [ ] 6.1 Implement `engine::ontology_scan` regex baseline
    ...
    - `RegexAnalyzer` (language-agnostic baseline, always available) detecting prohibited aliases as identifiers and emitting violations citing the owning constraint id with a remediation hint naming the correct term (e.g. `is_deleted` → C-02)
  ...
  - [ ]* 6.2 Write property test for the ontology gate
    ...
    - Assert every prohibited alias occurring as an identifier yields ≥1 violation citing the owning constraint id; include the `is_deleted` → C-02 case with the exact remediation string
...
- [ ] 8. Skills tools — tiered get_skill + ported legacy catalog
  ...
  - [ ] 8.2 Port the legacy catalog tools in `tools/skills.rs`
    ...
    - `read_skill` parses frontmatter/headings/sections and errors on missing/unresolved/parse-failure; `search_skills` case-insensitive substring (whole-string when `exact`) with empty-query error and empty-result set; `get_git_context` action status|log|diff (default status) with unsupported-action error; `build_skill_graph` builds and persists the graph queried by `read_graph`/`search_nodes`/`open_nodes`, which error when no graph is persisted
...
- [ ] 10. Gate tool — verify_gate
  - [ ] 10.1 Implement `tools/gates.rs`
    ...
    - Default execute mode runs the project verify/test command in the sandbox; evidence mode skips execution and requires `test_evidence`, rejecting the call when it is absent
...
- [ ] 11. TDD tool — tdd_cycle
  - [ ] 11.1 Implement `tools/tdd.rs`
    ...
    - Enforce Red→Green→Refactor ordering; reject out-of-order transitions with an invalid-transition error leaving recorded TDD state unchanged
...
- [ ] 12. Ontology tools — generate_ontology and verify_ontology
  - [ ] 12.1 Implement `tools/ontology.rs`
    - `truenorth_generate_ontology` (schemars contract: `domain` 1–200 chars, `source_paths` ≥1 entry); on success write `specs/ontology.yaml` with entities (invariants, states, transitions, prohibited aliases) and global constraints; reject invalid input identifying the parameter without writing; on write failure leave any existing file unchanged; seed as a new file only, erroring when `specs/ontology.yaml` already exists
    - `truenorth_verify_ontology` (schemars contract: optional `scope_paths`) delegating to `engine::ontology_scan`; default scope to git-changed files in scope; return passed with zero files scanned when scope is empty; return passed when no violations; otherwise return the formatted first-violation error
...
- [ ] 13. Checkpoint - Ensure all engine and tool tests pass
...
- [ ] 17. Checkpoint - Ensure runtime parity suite passes
  - Ensure all tests pass, ask the user if questions arise.
...
- [ ] 22. Final checkpoint - Ensure all tests pass and no dangling references
  - Ensure all tests pass, ask the user if questions arise.
...
- Checkpoints ensure incremental validation; task 20 is the parity gate that must hold before the destructive cleanup in task 21 begins (parity-before-removal, Req 10.5 / §9.3).
