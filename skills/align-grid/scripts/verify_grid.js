...
const url =
  args.find((a) => !a.startsWith('--')) || 'file://' + path.join(process.cwd(), 'index.html');
const opt = (k) => {
...
(async () => {
  ...
  for (const W of widths) {
    ...
    const res = await page.evaluate((BL) => {
      ...
      const nearest = (v, arr) =>
        arr.reduce((b, e) => (Math.abs(e - v) < Math.abs(b - v) ? e : b), arr[0]);
      ...
      return {
        ...
        fontFamily: getComputedStyle(
          document.querySelector('.masthead') || document.body,
        ).fontFamily.split(',')[0],
      };
    }, BL);
