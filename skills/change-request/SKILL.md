...
4. **Place**: Append story under an existing epic capsule, or create a new capsule and register it in `.agent/tasks/release-plan.yml` `epics[]`.
...
2. **Re-sort** `.agent/tasks/release-plan.yml` `epics[]` and per-epic `wsjf` fields.
3. **Flag cut candidates**: WSJF < 1.5.
4. **Update** `.agent/tasks/release-plan.yml` and epic `wsjf` keys with rationale.
5. **Report** the delta.
...
2. **Locate**: Identify which epic or capability in the task group the request affects or replaces.
...
Update `.agent/tasks/execution-status.yml` from the epic manifests. Suggest `plan-work` or
